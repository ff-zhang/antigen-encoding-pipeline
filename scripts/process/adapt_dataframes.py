...
def return_data_date_dict():
    splitPath = os.getcwd().split('/')
    rootIndex = splitPath.index('antigen-encoding-pipeline')
...
def main():
    for file in os.listdir(folder):
        if '.pkl' in file or '.hdf' in file:
            if '.pkl' in file:
                tmp = pd.read_pickle(folder + file)
            elif '.hdf' in file:
                tmp = pd.read_hdf(folder + file, key='df')
            # filename=file[41:-13]
            ...
            for idx, level_name in enumerate(df.columns):
                if level_name in level_name_changes.keys():
                    df.columns = list(df.columns[:idx]) + [level_name_changes[level_name]] + list(
                        df.columns[idx + 1:])

            print(filename)
            new_file = file.split('.')[0] + "-final.pkl"
            ...
            if filename in tumor_timeseries[:2]:
                ...
                df["IFNgPulseConcentration"][df.APCType == "Tumor"] = [conc for conc in
                                                                       df.Concentration[
                                                                           df.APCType == "Tumor"]]
                ...
                df["IFNgPulseConcentration"] = [conc[:3] if "IFNg" in conc else "None" for conc in
                                                df.Concentration]
                df["Concentration"] = ["None" if "IFNg" in conc else conc for conc in
                                       df.Concentration]
                ...
                df = df.iloc[:,
                     ::-1]  # Hacky shortcut to get ordering of columns that results in similar level order for APCType, APC, IFNgPulseConcentration
            ...
            # [(Experiment=None)&(Antagonist=None),(Agonist_Antagonist_Locations=Blank)]
                ...
                if "TCellType" in df.columns:
                    ...
                    df["TCellType"] = ["P14" if peptide in ["A3V", "AV", "gp33WT"] else "OT1" for
                                       peptide in df.Peptide]
            ...
            df = df.reorder_levels([level for level in df.index.names if
                                    level not in standard_levels] + standard_levels)
