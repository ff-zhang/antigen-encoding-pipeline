...
class InputDatasetSelectionPage(tk.Frame):
    def __init__(self, master):
        ...
        dataset = pd.DataFrame(dataset['value'].values,
                               index=pd.MultiIndex.from_frame(dataset.iloc[:, :-1]),
                               columns=['value'])
        ...
        titleLabel = tk.Label(titleWindow, text='Training set name:', pady=10,
        ...
        timeLabel = tk.Label(timeWindow, text='Time range (START TIME-END TIME):', pady=10,
                             font='Helvetica 18 bold').grid(row=0, column=0)
        ...
        l1 = tk.Label(labelWindow, text='Parameters:', pady=10, font='Helvetica 18 bold').grid(
            row=0, column=0, columnspan=len(trueLabelDict) * 6)
        ...
        def collectInputs(dataset):
            ...
            print(dataset)
            df = dataset.loc[tuple(includeLevelValueList), :].unstack(['Feature', 'Cytokine']).loc[
                 :, 'value']
            print(df)

            trainingSetName = e1.get()
            # Save min/max and normalize data
            pickle.dump([df.min(), df.max()],
                        open(path + "output/trained-networks/min_max-" + trainingSetName + ".pkl",
                             "wb"))
            ...
            score = mlp.score(df, y);
            print("Training score %.1f" % (100 * score));
            pickle.dump(mlp, open(path + "output/trained-networks/mlp-" + trainingSetName + ".pkl",
                                  "wb"))

            df_WT_proj = pd.DataFrame(np.dot(df, mlp.coefs_[0]), index=df.index,
                                      columns=["Node 1", "Node 2"])
            df_WT_proj.to_pickle(
                path + "output/trained-networks/proj-WT-" + trainingSetName + ".pkl")
        ...
        tk.Button(buttonWindow, text="OK", command=lambda: collectInputs(dataset)).grid(
            row=maxNumLevelValues + 4, column=0)
        tk.Button(buttonWindow, text="Back",
                  command=lambda: master.switch_frame(master.homepage)).grid(
            row=maxNumLevelValues + 4, column=1)
        tk.Button(buttonWindow, text="Quit", command=lambda: quit()).grid(row=maxNumLevelValues + 4,
                                                                          column=2)
...
def plot_weights(mlp, cytokines, peptides, **kwargs):
    ...
    ax[0].set(xlabel="Cytokine", xticks=np.arange(len(cytokines)), xticklabels=cytokines,
              ylabel="Weights")
