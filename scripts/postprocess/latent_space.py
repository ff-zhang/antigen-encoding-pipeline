...
class WeightMatrixSelectionPage(tk.Frame):
    def __init__(self, master, lspB, nsp):
        ...
        l1 = tk.Label(labelWindow, text="Select trained weight matrix to project with:",
        ...
        for i, dataset in enumerate(trainingDatasets):
            rb = tk.Radiobutton(mainWindow, text=dataset, padx=20, variable=datasetVar,
                                value=dataset)
        ...
        def collectInputs():
            ...
            # Grab weight matrix
            mlp = pickle.load(
                open(path + 'output/trained-networks/mlp-' + datasetName + '.pkl', "rb"))
            global weightMatrix
            weightMatrix = mlp
            # Grab training set min/max to normalize df to be projected
            global df_min, df_max
            df_min, df_max = pd.read_pickle(
                path + "output/trained-networks/min_max-" + datasetName + ".pkl")
...
class WTorMutantDatasetSelectionPage(tk.Frame):
    def __init__(self, master):
        ...
        datasetRadioButtons = []
        # Mutant List
        mutantTypeList = ["Tumor", "Activation", "TCellNumber", "DifferentAPC", "OT1CAR",
                          "DifferentTCR", "DrugPerturbation", "HighMI", "hTCR"]
        ...
        def collectInputs():
            ...
            if datasetType == 'WT':
                ...
                featureColumns = pd.read_pickle(
                    path + "output/trained-networks/train-" + datasetName + ".pkl").columns
                ...
                df_mutant_proj = pd.DataFrame(np.dot(df_mutant, weightMatrix.coefs_[0]),
                                              index=df_mutant.index, columns=["Node 1", "Node 2"])
                ...
                df_mutant_proj = set_standard_order(df_mutant_proj.copy().reset_index(),
                                                    returnSortedLevelValues=False)
                df_mutant_proj = pd.DataFrame(df_mutant_proj.iloc[:, -2:].values,
                                              index=pd.MultiIndex.from_frame(
                                                  df_mutant_proj.iloc[:, :-2]),
                                              columns=['Node 1', 'Node 2'])
                with open(
                        path + 'output/projected-dataframes/projection-' + projectionName + '.pkl',
                        'wb') as f:
                ...
                # switch to choosing fit type, then plotting
                    ...
                    master.switch_frame(nextSwitchPage, df_mutant_proj,
                                        WTorMutantDatasetSelectionPage)
        ...
        tk.Button(buttonWindow, text="OK", command=lambda: collectInputs()).pack(in_=buttonWindow,
                                                                                 side=tk.LEFT)
        tk.Button(buttonWindow, text="Back",
                  command=lambda: master.switch_frame(WeightMatrixSelectionPage, latentSpaceBool,
                                                      nextSwitchPage)).pack(in_=buttonWindow,
                                                                            side=tk.LEFT)
        tk.Button(buttonWindow, text="Quit", command=lambda: quit()).pack(in_=buttonWindow,
                                                                          side=tk.LEFT)
...
class TrainingDatasetSelectionPage(tk.Frame):
    def __init__(self, master):
        ...
        labelWindow = tk.Frame(self)
        l1 = tk.Label(labelWindow, text="Select training dataset to project on:",
                      font='Helvetica 18 bold').pack()
        labelWindow.pack(side=tk.TOP, padx=10, pady=10)
        ...
        idx = pd.IndexSlice
        sortedWT = set_standard_order(
            all_df_WT.loc[idx[:, '100k', :, :, 1, 'IFNg', 'concentration'], :].reset_index())
        ...
        def collectInputs():
            ...
            df_WT_proj = pd.DataFrame(np.dot(df_WT, weightMatrix.coefs_[0]), index=df_WT.index,
                                      columns=["Node 1", "Node 2"])
            # switch to plotting
            projectionName = '-'.join([datasetName, datasetName2])
            with open(path + 'scripts/gui/plotting/projectionName.pkl', 'wb') as f:
                pickle.dump(projectionName, f)
            df_WT_proj = set_standard_order(df_WT_proj.copy().reset_index(),
                                            returnSortedLevelValues=False)
            df_WT_proj = pd.DataFrame(df_WT_proj.iloc[:, -2:].values,
                                      index=pd.MultiIndex.from_frame(df_WT_proj.iloc[:, :-2]),
                                      columns=['Node 1', 'Node 2'])
            with open(path + 'output/projected-dataframes/projection-' + projectionName + '.pkl',
                      'wb') as f:
        ...
        tk.Button(buttonWindow, text="OK", command=lambda: collectInputs()).pack(in_=buttonWindow,
                                                                                 side=tk.LEFT)
        tk.Button(buttonWindow, text="Back",
                  command=lambda: master.switch_frame(WTorMutantDatasetSelectionPage)).pack(
...
def import_WT_output():
    ...
    for file in os.listdir(folder):
        ...
        df = pd.concat([df[mask]], keys=[file[:-4]],
                       names=["Data"])  # add experiment name as multiindex level
...
def import_mutant_output(mutant):
    ...
    for file in os.listdir(folder):
        ...
        if "df_full" not in locals():
            ...
            df_full = pd.concat([df_full, df],
                                levels=["Data"] + mutant_levels[mutant] + essential_levels)
