...
color_dict = {var: color for var, color in
              zip(tcellnumbers + peptides + concentrations, colors + colors + colors)}
marker_dict = {var: marker for var, marker in
               zip(tcellnumbers + peptides + concentrations, markers + markers + markers)}
size_dict = {var: size for var, size in
             zip(tcellnumbers + peptides + concentrations, sizes + sizes + sizes)}
...
def extract_features(df):
    ...
    for idx in df.groupby(["Data", "TCellNumber", "Peptide"]).size().index:
        ...
        for idz in df.loc[idx].index:
            ...
            df.loc[idxz, "Dv"] = (
                        df.loc[idxy, "v_0"] - df.loc[idxz, "v_0"])  # *np.sqrt(df.loc[idxz,"v_0"])
...
def plot_correlating_variables(df_all_params):
    ...
    h = sns.relplot(data=df_full.reset_index(), x="Peptide", y="v_0", hue="Concentration",
                    hue_order=concentrations, col="Data", col_wrap=3)
    ...
    index = df_full.Concentration.map(
        {conc: i for conc, i in zip(concentrations, range(4))}).sort_values().index
...
def plot_features_in_3d_space(params):
    ...
    mscatter(params["Dv"], params["v0/t0"], params["v_0"], ax=ax,
             ...
             depthshade=False)
    ax.set(xlabel='Dv', ylabel='v0 / t0', zlabel='v0', ylim=[0, 0.6],
           title="Color by T cell number")
