...
def plot(plottingDf, subsettedDf, kwargs, facetKwargs, auxillaryKwargs, plotOptions):
    ...
    if auxillaryKwargs['subPlotType'] == 'histogram':
        ...
        if auxillaryKwargs['dataType'] != 'singlecell':
            ...
            uniqueKwargCombinations = [list(x) for x in set(
                tuple(x) for x in list(plottingDf.iloc[:, kwargIndices].values))]
            hist = [0]
            indexTuples = []
            ...
            smoothedHistBins = savgol_filter(hist,
                                             auxillaryKwargs['plotspecifickwargs']['smoothing'] - 1,
                                             2)
            ...
            mi = pd.MultiIndex.from_tuples(indexTuples, names=itemgetter(*kwargIndices)(
            ...
            fg = sns.relplot(data=plottingDf, kind='line', x='MFI', y=columns[1],
