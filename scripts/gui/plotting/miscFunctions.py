...
def Hill(x, Amplitude, EC50, hill, Background):
    return np.log10(
        Amplitude * np.power(x, hill) / (np.power(EC50, hill) + np.power(x, hill)) + Background)
...
def logisticDoubleExponential(x, amplitude, tau1, tau2, timedelay1, timedelay2, vshift):
    return amplitude * np.subtract(
        np.divide(1, np.add(1, np.exp(np.multiply(-1 * tau1, np.subtract(x, timedelay1))))),
        np.divide(1,
                  np.add(1, np.exp(np.multiply(-1 * tau2, np.subtract(x, timedelay2)))))) + vshift
...
def cleanUpFlowjoCSV(fileArray, folderName, dataType, experimentParameters):
    ...
    for name in fileArray:
        ...
        for i in range(0, temp.shape[0]):
            fullfilename = 'inputData/singleCellCSVFiles/' + name + '/' + temp.iloc[i, 0][
                                                                          :temp.iloc[i, 0].find(
                                                                              '.')]
...
unitPrefixDictionary = {'fM': 1e-15, 'pM': 1e-12, 'nM': 1e-9, 'uM': 1e-6, 'mM': 1e-3, 'M': 1e0,
                        '': 0, 'K': 1000}
        ...
        if ('-' in inputString):  # - and ,
            ...
            for experimentRangeString in experimentRanges:
                if ('-' in experimentRangeString):
                    ...
                    tempExperimentNumbers = list(
                        range(experimentNumberRange[0], experimentNumberRange[1] + 1))
...
def reindexDataFrame(dfToReindex, indexdf, singlecellToNonSinglecell, sortDataTypeLevels=True):
    ...
    if sortDataTypeLevels:
        if indexdf.index.names[0] in ['Cytokine', 'Statistic']:
            ...
            indexingDf = indexdf.loc[pd.unique(indexdf.index.get_level_values(0))[0]].loc[
                pd.unique(indexdf.index.get_level_values(1))[0]].loc[
                pd.unique(indexdf.index.get_level_values(2))[0]]
        else:
            indexingDf = indexdf
    else:
        indexingDf = indexdf
    ...
    if not singlecellToNonSinglecell:
        ...
        reindexedDf = pd.DataFrame(reindexedDfMatrix, index=indexingDf.index,
                                   columns=dfToReindex.columns)
    else:
        ...
        while k < dfToReindex.shape[0]:
            ...
            row += 1
            k += stackedLength
        reindexedMultiIndex = pd.MultiIndex.from_tuples(reindexedLevelNames,
                                                        names=dfToReindex.index.names)
        reindexedDf = pd.DataFrame(reindexedDfMatrix, index=reindexedMultiIndex,
                                   columns=dfToReindex.columns)
    return reindexedDf
...
def returnGates(logicleData, rawData, generationZeroBoundary, numGens):
    ...
    for generation in range(maxGenerationNumber - 1):
        generationGatesLog.append(
            (generationMeansLog[generation] + generationMeansLog[generation + 1]) * 0.5)
...
def extractValues(currentLevelLayout, valueToRemove, equalityBoolean):
    ...
    if equalityBoolean:
        idx2 = np.argwhere(
            np.all(currentLevelLayoutExtractedColumns[:, ...] == valueToRemove, axis=1))
    else:
        idx2 = np.argwhere(
            np.all(currentLevelLayoutExtractedColumns[:, ...] != valueToRemove, axis=1))
