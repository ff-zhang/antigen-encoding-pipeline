...
from miscFunctions import sortSINumerically, reindexDataFrame, setMaxWidth, \
    returnSpecificExtensionFiles, returnTicks, get_cluster_centroids
...
def createParameterSelectionRadiobuttons(radiobuttonWindow, parameterList, parameterValueDict):
    ...
    for i, parameter in enumerate(parameterList):
        ...
        for j in range(len(parameterValues)):
            ...
            radiobutton = tk.Radiobutton(radiobuttonWindow, text=parameterValue,
                                         value=parameterValue, variable=parameterVar)
...
def createParameterAdjustmentSliders(sliderWindow, parameterList, parameterBoundsDict):
    ...
    for i, parameter in enumerate(parameterList):
        ...
        slider = tk.Scale(sliderWindow, from_=parameterBounds[0], to=parameterBounds[1],
                          resolution=parameterBounds[2])
...
def createParameterSelectionDropdowns(dropdownWindow, parameterList, parameterValueDict,
                                      defaultParameterValueDict):
    dropdownList = []
    dropdownVarsDict = {}
    for i, parameter in enumerate(parameterList):
        parameterValues = parameterValueDict[parameter]
...
def returnOriginalOrders(trueLabelDict, plottingDf, kwargs, dimensionality):
    ...
    for kwarg in newkwargs:
        if newkwargs[kwarg] == 'Cluster' and '$' not in plottingDf[newkwargs[kwarg]][0]:
            orderedValues = list(
                map(str, sorted(list(map(int, list(pd.unique(plottingDf['Cluster'])))))))
        else:
...
def updateDropdownControlledCompositionPlot(frameCanvas, plotAxis, plottingDf, trueLabelDict,
                                            levelVars, legendoffset=1.7):
    plotAxis.clear()
    if not isinstance(plottingDf, list):
        ...
        # scatter plot (2d)
        if 'x' in newkwargs and 'y' in newkwargs and newkwargs['y'] not in ['frequency',
                                                                            'log-frequency',
                                                                            'percent']:
            ...
            if 'hue' in newkwargs:
                if newkwargs['hue'] in list(pd.unique(plottingDf['Feature'])):
                    ...
                    if newkwargs['hue'] == 'Time':
                        ...
                        palette = sns.color_palette(sns.color_palette(),
                                                    len(pd.unique(plottingDf[newkwargs['hue']])))
            ...
            g3 = sns.scatterplot(data=plottingDf, **newkwargs, **hue_orderDict, **paletteKwargs,
                                 s=5, alpha=0.7, **hueNormKwargs)
            leg = g3.legend(loc='center right', bbox_to_anchor=(legendoffset, 0.5), ncol=1,
                            framealpha=0)
        ...
        # barplot (1.5d) or kde (1d)
            ...
            if not featureBool:
                palette = sns.color_palette(sns.color_palette(),
                                            len(pd.unique(plottingDf['Cluster'])))
            else:
                if 'hue' in newkwargs:
                    palette = sns.color_palette(sns.color_palette(),
                                                len(pd.unique(plottingDf[newkwargs['hue']])))
            ...
            if yaxisFeature in ['frequency', 'log-frequency']:
                if is_number(plottingDf[newkwargs['x']][0]) and len(
                        pd.unique(plottingDf[newkwargs['x']])) > 4 and newkwargs['x'] != 'Cluster':
                    if not featureBool:
                        # "Unstack" dataframe
                        plottingDf = plottingDf[plottingDf['Feature'] == plottingDf['Feature'][0]]
                        for i, cluster in enumerate(orderedClusters):
                            g3 = sns.kdeplot(
                                plottingDf[plottingDf['Cluster'] == cluster][newkwargs['x']],
                                color=palette[i], ax=plotAxis, shade=True, label=cluster)
                    else:
                        ...
                        for i, cluster in enumerate(hue_order):
                            g3 = sns.kdeplot(
                                plottingDf[plottingDf[newkwargs['hue']] == cluster][newkwargs['x']],
                                color=palette[i], ax=plotAxis, shade=True, label=cluster)
                        if max(plottingDf['Metric']) > 100:
                            tickDict = {}
                            ticks = [-1000, 100, 1000, 10000, 100000]
                            ...
                            if 'hue' in newkwargs:
                                ...
                                for hueval in pd.unique(plottingDf[newkwargs['hue']]):
                                    subplotValues.append(
                                        plottingDf[plottingDf[newkwargs['hue']] == hueval][
                                            'Metric'])
                            else:
                    ...
                    # "Unstack" dataframe
                    plottingDf = plottingDf[plottingDf['Feature'] == plottingDf['Feature'][0]]
                    g3 = sns.countplot(data=plottingDf, ax=plotAxis, **newkwargs, **orderDict,
                ...
                if 'hue' in newkwargs.keys():
                    ...
                    plottingDf = plottingDf.groupby(x)[y].value_counts(normalize=True).mul(
                ...
                g3 = sns.barplot(data=plottingDf, ax=plotAxis, **newkwargs, **orderDict,
                                 edgecolor='black', linewidth=1)
                # Log frequency if needed
                if yaxisFeature == 'log-frequency':
                    plotAxis.set_yscale('log')
            ...
            if kdePlotBool:
                if newkwargs['hue'] == 'Cluster':
                    newkwargs['hue'] = 'Group'
                leg = g3.legend(loc='center right', bbox_to_anchor=(legendoffset, 0.5), ncol=1,
                                framealpha=0, title=newkwargs['hue'])
            else:
                leg = g3.legend(loc='center right', bbox_to_anchor=(legendoffset, 0.5), ncol=1,
                                framealpha=0)
...
def updateDropdownControlledPlot(frameCanvas, plotAxis, plottingDf, levelVars, xColumn, yColumn,
                                 alpha=0.3, legendoffset=-0.1, trueLabelDict=[]):
    plotAxis.clear()
    ...
    legendSpillover = 15
    leg = g3.legend(loc='center right', bbox_to_anchor=(legendoffset, 0.5),
                    ncol=math.ceil(numLegendElements / legendSpillover), framealpha=0)
    if featureHueBool:
        a, b = returnTicks([-1000, 100, 10000, 100000])
        for t, l in zip(leg.texts[1:], (b)):
            t.set_text(l)
    ...
    if len(sortedNumUniqueElements) > 2:
        ...
        if len(columnsLeftToAssign) > 0:
            if sortedNumUniqueElements2[0] > 1:
                thirdMaxUniqueElementsColumn = columnsLeftToAssign[
                    numUniqueElements2.index(sortedNumUniqueElements2[0])]
