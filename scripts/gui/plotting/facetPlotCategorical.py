...
def plot(plottingDf, subsettedDf, kwargs, facetKwargs, auxillaryKwargs, plotOptions):
    ...
    if auxillaryKwargs['subPlotType'] == 'point':
        if auxillaryKwargs['addDistributionPoints']:
            fg = sns.catplot(**kwargs, **facetKwargs, data=plottingDf,
                             kind=auxillaryKwargs['subPlotType'], ci=errorBar, join=False,
                             color='k', capsize=0.05, markers='_', zorder=3, errwidth=1,
                             **plotOptions['X']['figureDimensions'])
        else:
            fg = sns.catplot(**kwargs, **facetKwargs, data=plottingDf,
                             kind=auxillaryKwargs['subPlotType'], ci=errorBar, join=False,
                             capsize=0.05, errwidth=1, **plotOptions['X']['figureDimensions'])
    elif auxillaryKwargs['subPlotType'] == 'box':
        fg = sns.catplot(**kwargs, **facetKwargs, data=plottingDf,
                         kind=auxillaryKwargs['subPlotType'])
    elif auxillaryKwargs['subPlotType'] == 'bar':
        fg = sns.catplot(**kwargs, **facetKwargs, data=plottingDf,
                         kind=auxillaryKwargs['subPlotType'], ci=errorBar, alpha=0.8, errwidth=1,
                         capsize=0.05, **plotOptions['X']['figureDimensions'])
    # violin,swarm,strip
    elif auxillaryKwargs['subPlotType'] == 'violin':
        fg = sns.catplot(**kwargs, facet_kws=facetKwargs, data=plottingDf,
                         kind=auxillaryKwargs['subPlotType'], alpha=0.8, scale='width',
                         **plotOptions['X']['figureDimensions'])
        ...
        fg = sns.catplot(**kwargs, facet_kws=facetKwargs, data=plottingDf,
                         kind=auxillaryKwargs['subPlotType'], alpha=0.7, edgecolor='black',
                         linewidth=0.3, dodge=True, **plotOptions['X']['figureDimensions'])
    if auxillaryKwargs['addDistributionPoints']:
        NoneType = type(None)
            ...
            for rowVal in pd.unique(plottingDf[kwargs['row']]):
                for colVal in pd.unique(plottingDf[kwargs['col']]):
                    ...
                    if auxillaryKwargs['subPlotType'] != 'violin' and swarm == False:
                        a = sns.stripplot(**secondkwargs, data=secondPlottingDf,
                                          ax=fg.fig.axes[axisIndex])
                    else:
                        a = sns.swarmplot(**secondkwargs, data=secondPlottingDf,
                                          ax=fg.fig.axes[axisIndex])
    ...
    if auxillaryKwargs['subPlotType'] in ['swarm', 'strip'] and plotOptions['Y'][
        'axisScaling'] == 'Logarithmic':
        allyticks, allyticklabels, allminoryticks = returnLogYTicksAndLabels(
            plottingDf[kwargs['y']])
        ...
        for axis in plotOptions:
            ...
            if 'Y' in axis:
                if plotOptions[axis]['axisScaling'] == 'Logarithmic':
                    ...
                    for i in range(k):
                        fg.fig.get_axes()[i].set_yscale('symlog', linthreshx=plotOptions[axis][
                            'linThreshold'])

                if str(plotOptions[axis]['limit'][0]) != '' or str(
                        plotOptions[axis]['limit'][1]) != '':
                    for i in range(k):
                        if str(plotOptions[axis]['limit'][0]) != '' and str(
                                plotOptions[axis]['limit'][1]) != '':
                            fg.fig.get_axes()[i].set_ylim(
                                bottom=float(plotOptions[axis]['limit'][0]),
                                top=float(plotOptions[axis]['limit'][1]))
                        else:
                ...
                if plotOptions[axis]['axisScaling'] == 'Logarithmic':
                    ...
                    for i in range(k):
                        fg.fig.get_axes()[i].set_xscale('symlog', linthreshx=plotOptions[axis][
                            'linThreshold'])
