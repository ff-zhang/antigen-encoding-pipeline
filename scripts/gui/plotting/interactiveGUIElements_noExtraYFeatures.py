...
from miscFunctions import sortSINumerically, reindexDataFrame, setMaxWidth, \
    returnSpecificExtensionFiles, returnTicks
...
def addLogicleAxes(axis, ticks):
    ...
    xtickValues, xtickLabels = returnTicks(ticks['x'])
    ytickValues, ytickLabels = returnTicks(ticks['y'])
    for ax in ticks:
        if ax == 'x':
...
def updateDropdownControlledCompositionPlot(frameCanvas, plotAxis, plottingDf, trueLabelDict,
                                            levelVars, compositionParameter, legendoffset=1.7):
    plotAxis.clear()
    if not isinstance(plottingDf, list):
        ...
        # scatter plot (2d)
        if 'x' in newkwargs and 'y' in newkwargs:
