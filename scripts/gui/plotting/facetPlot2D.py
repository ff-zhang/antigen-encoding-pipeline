    ...
    if 'Time' in plottingDf.columns:
        if len(pd.unique(plottingDf.Time)) > 36:
            if kwargs['x'] == 'Time' or kwargs['y'] == 'Time':
                print('wat1')
                fg = sns.relplot(data=plottingDf, kind=auxillaryKwargs['subPlotType'],
                                 facet_kws=facetKwargs, ci=False, **kwargs,
                                 **plotOptions['X']['figureDimensions'])
            else:
                print('wat2')
                fg = sns.relplot(data=plottingDf, kind=auxillaryKwargs['subPlotType'],
                                 facet_kws=facetKwargs, ci=False, **kwargs,
                                 **plotOptions['X']['figureDimensions'], sort=False, mew=0, ms=4)
        else:
            if 'style' not in kwargs.keys():
                print('wat3')
                fg = sns.relplot(data=plottingDf, marker='o', kind=auxillaryKwargs['subPlotType'],
                                 facet_kws=facetKwargs, ci=False, **kwargs,
                                 **plotOptions['X']['figureDimensions'], sort=False, mew=0, ms=4)
            else:
                print('wat4')
                fg = sns.relplot(data=plottingDf, markers=True, kind=auxillaryKwargs['subPlotType'],
                                 facet_kws=facetKwargs, ci=False, **kwargs,
                                 **plotOptions['X']['figureDimensions'], sort=False, mew=0, ms=4)
    else:
    ...
    for axis in plotOptions:
        ...
        if 'Y' in axis:
            if plotOptions[axis]['axisScaling'] == 'Logarithmic':
                ...
                for i in range(k):
                    fg.fig.get_axes()[i].set_yscale('symlog',
                                                    linthreshx=plotOptions[axis]['linThreshold'])
            ...
            if plotOptions[axis]['axisScaling'] == 'Logarithmic':
                ...
                for i in range(k):
                    fg.fig.get_axes()[i].set_xscale('symlog',
                                                    linthreshx=plotOptions[axis]['linThreshold'])
