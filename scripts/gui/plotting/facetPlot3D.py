    ...
    for level in range(len(list(df.index.names)))[::-1]:
        ...
        for label, rpos in label_len(df.index, level):
            ...
            if (label not in list(list(df.index.names)[-1])):
                t = ax.text(xpos + (dividerLength / 2), lypos, label, va='center', ha='center',
                            transform=ax.transAxes, rotation=0)
            else:
                t = ax.text(-1 * (dividerLength / 2), lypos, label, va='center', ha='center',
                            transform=ax.transAxes, rotation=0)
...
def label_columns(ax, df):
    ...
    for timepoint in df.columns.values:
        ...
        ax.text(xpos + scale / 2, ypos / 2, str(timepoint), ha='center', va='center',
...
def label_headers(ax, df):
    ...
    for name in df.index.names:
        ax.text(xpos + (dividerLength / 2), 1 + (lineheight - 1) / 2, name, ha='center',
                va='center', transform=ax.transAxes, size='x-small')
        xpos += dividerLength
...
def draw_faceted_heatmap(data, indexingdf, xaxis, yaxis, zaxis, lognorm, cbarticks, logarithmic,
                         symlog, symlognorm, linthresh, **kwargs):
    ...
    if logarithmic:
        g = sns.heatmap(data, norm=lognorm, **kwargs, cbar=True,
                        cbar_kws={"ticks": cbarticks, 'label': zaxis})
    elif symlog:
        linthresh = int(linthresh)
        maxlog = int(np.ceil(np.log10(data.values.max())))
        minlog = int(np.ceil(np.log10(-1 * data.values.min())))
        tick_locations = ([-(10 ** x) for x in range(-linthresh, minlog + 1, 1)][::-1] + [0.0] + [
            (10 ** x) for x in range(-linthresh, maxlog + 1, 1)])
        # generate logarithmic ticks
        g = sns.heatmap(data, norm=symlognorm, cbar_kws={'label': zaxis, 'ticks': tick_locations,
...
def plot(plottingDf, subsettedDf, kwargs, facetKwargs, auxillaryKwargs, plotOptions):
    if auxillaryKwargs['subPlotType'] == 'heatmap':
        ...
        fg = sns.FacetGrid(plottingDf, height=h, aspect=a, gridspec_kws={"wspace": 0.4},
                           **auxillaryKwargs['facetgridkwargs'])
        if plotOptions['Colorbar']['axisScaling'] == 'Logarithmic':
            logbool = True
            symlogbool = False
            ...
            log_norm = LogNorm(vmin=subsettedDf[kwargs['z']].min().min(),
                               vmax=subsettedDf[kwargs['z']].max().max())
            sym_log_norm = ''
            cbar_ticks = [math.pow(10, i) for i in
                          range(math.floor(math.log10(subsettedDf[kwargs['z']].min().min())),
                                1 + math.ceil(math.log10(subsettedDf[kwargs['z']].max().max())))]
            lin_thresh = ''
        ...
        fg.map_dataframe(draw_faceted_heatmap, indexingdf=subsettedDf, xaxis=kwargs['x'],
                         yaxis=kwargs['y'], zaxis=kwargs['z']
                         , lognorm=log_norm, cbarticks=cbar_ticks, logarithmic=logbool,
                         symlog=symlogbool, symlognorm=sym_log_norm, linthresh=lin_thresh)
